import {Category} from "../models/category";

export class DefaultCategories {
  public static readonly TOB: Category = {
    "name": "TOB",
    locked: true,
    "items": [
      {
        "id": 22326,
        "name": "Justiciar faceguard",
        "unlocked": false
      },
      {
        "id": 22325,
        "name": "Scythe of vitur",
        "unlocked": false
      },
      {
        "id": 22327,
        "name": "Justiciar chestguard",
        "unlocked": false
      },
      {
        "id": 22484,
        "name": "Ghrazi rapier",
        "unlocked": false
      },
      {
        "id": 22328,
        "name": "Justiciar legguards",
        "unlocked": false
      },
      {
        "id": 22323,
        "name": "Sanguinesti staff",
        "unlocked": false
      },
      {
        "id": 22322,
        "name": "Avernic defender",
        "unlocked": false
      }
    ]
  };

  public static readonly COX: Category = {
    "name": "COX",
    locked: true,
    "items": [
      {
        "id": 21018,
        "name": "Ancestral hat",
        "unlocked": false
      },
      {
        "id": 21015,
        "name": "Dinh's bulwark",
        "unlocked": false
      },
      {
        "id": 21000,
        "name": "Twisted buckler",
        "unlocked": false
      },
      {
        "id": 21021,
        "name": "Ancestral robe top",
        "unlocked": false
      },
      {
        "id": 13652,
        "name": "Dragon claws",
        "unlocked": false
      },
      {
        "id": 21012,
        "name": "Dragon hunter crossbow",
        "unlocked": false
      },
      {
        "id": 21024,
        "name": "Ancestral robe bottom",
        "unlocked": false
      },
      {
        "id": 21034,
        "name": "Dexterous prayer scroll",
        "unlocked": false
      },
      {
        "id": 21079,
        "name": "Arcane prayer scroll",
        "unlocked": false
      },
      {
        "id": 21043,
        "name": "Kodai insignia",
        "unlocked": false
      },
      {
        "id": 21003,
        "name": "Elder maul",
        "unlocked": false
      },
      {
        "id": 20997,
        "name": "Twisted bow",
        "unlocked": false
      }
    ]
  };

  public static readonly PETS: Category = {
    "name": "Pets",
    locked: true,
    "items": [
      {
        "id": 13262,
        "name": "Abyssal orphan",
        "unlocked": false
      },
      {
        "id": 12646,
        "name": "Baby mole",
        "unlocked": false
      },
      {
        "id": 13178,
        "name": "Callisto cub",
        "unlocked": false
      },
      {
        "id": 13247,
        "name": "Hellpuppy",
        "unlocked": false
      },
      {
        "id": 22746,
        "name": "Ikkle hydra",
        "unlocked": false
      },
      {
        "id": 21291,
        "name": "Jal-nib-rek",
        "unlocked": false
      },
      {
        "id": 12647,
        "name": "Kalphite princess",
        "unlocked": false
      },
      {
        "id": 22473,
        "name": "Lil' zik",
        "unlocked": false
      },
      {
        "id": 24491,
        "name": "Little nightmare",
        "unlocked": false
      },
      {
        "id": 21748,
        "name": "Noon",
        "unlocked": false
      },
      {
        "id": 20851,
        "name": "Olmlet",
        "unlocked": false
      },
      {
        "id": 11995,
        "name": "Pet chaos elemental",
        "unlocked": false
      },
      {
        "id": 12644,
        "name": "Pet dagannoth prime",
        "unlocked": false
      },
      {
        "id": 12645,
        "name": "Pet dagannoth rex",
        "unlocked": false
      },
      {
        "id": 12643,
        "name": "Pet dagannoth supreme",
        "unlocked": false
      },
      {
        "id": 12816,
        "name": "Pet dark core",
        "unlocked": false
      },
      {
        "id": 12650,
        "name": "Pet general graardor",
        "unlocked": false
      },
      {
        "id": 12652,
        "name": "Pet k'ril tsutsaroth",
        "unlocked": false
      },
      {
        "id": 12655,
        "name": "Pet kraken",
        "unlocked": false
      },
      {
        "id": 12649,
        "name": "Pet kree'arra",
        "unlocked": false
      },
      {
        "id": 12648,
        "name": "Pet smoke devil",
        "unlocked": false
      },
      {
        "id": 12921,
        "name": "Pet snakeling",
        "unlocked": false
      },
      {
        "id": 12651,
        "name": "Pet zilyana",
        "unlocked": false
      },
      {
        "id": 12653,
        "name": "Prince black dragon",
        "unlocked": false
      },
      {
        "id": 13181,
        "name": "Scorpia's offspring",
        "unlocked": false
      },
      {
        "id": 21273,
        "name": "Skotos",
        "unlocked": false
      },
      {
        "id": 23495,
        "name": "Sraracha",
        "unlocked": false
      },
      {
        "id": 13225,
        "name": "Tzrek-jad",
        "unlocked": false
      },
      {
        "id": 13177,
        "name": "Venenatis spiderling",
        "unlocked": false
      },
      {
        "id": 13179,
        "name": "Vet'ion jr.",
        "unlocked": false
      },
      {
        "id": 21992,
        "name": "Vorki",
        "unlocked": false
      },
      {
        "id": 13323,
        "name": "Baby chinchompa",
        "unlocked": false
      },
      {
        "id": 13322,
        "name": "Beaver",
        "unlocked": false
      },
      {
        "id": 20659,
        "name": "Giant squirrel",
        "unlocked": false
      },
      {
        "id": 13320,
        "name": "Heron",
        "unlocked": false
      },
      {
        "id": 20665,
        "name": "Rift guardian",
        "unlocked": false
      },
      {
        "id": 13321,
        "name": "Rock golem",
        "unlocked": false
      },
      {
        "id": 20663,
        "name": "Rocky",
        "unlocked": false
      },
      {
        "id": 20661,
        "name": "Tangleroot",
        "unlocked": false
      },
      {
        "id": 19730,
        "name": "Bloodhound",
        "unlocked": false
      },
      {
        "id": 13071,
        "name": "Chompy chick",
        "unlocked": false
      },
      {
        "id": 21509,
        "name": "Herbi",
        "unlocked": false
      },
      {
        "id": 12703,
        "name": "Pet penance queen",
        "unlocked": false
      },
      {
        "id": 20693,
        "name": "Phoenix",
        "unlocked": false
      },
      {
        "id": 23757,
        "name": "Youngllef",
        "unlocked": false
      },
      {
        "id": 23760,
        "name": "Smolcano",
        "unlocked": false
      }
    ]
  };

  public static readonly GWD: Category = {
    "name": "GWD",
    locked: true,
    "items": [
      {
        "id": 11826,
        "name": "Armadyl helmet",
        "unlocked": false
      },
      {
        "id": 11832,
        "name": "Bandos chestplate",
        "unlocked": false
      },
      {
        "id": 11824,
        "name": "Zamorakian spear",
        "unlocked": false
      },
      {
        "id": 11810,
        "name": "Armadyl hilt",
        "unlocked": false
      },
      {
        "id": 11785,
        "name": "Armadyl crossbow",
        "unlocked": false
      },
      {
        "id": 11828,
        "name": "Armadyl chestplate",
        "unlocked": false
      },
      {
        "id": 11834,
        "name": "Bandos tassets",
        "unlocked": false
      },
      {
        "id": 11787,
        "name": "Steam battlestaff",
        "unlocked": false
      },
      {
        "id": 11812,
        "name": "Bandos hilt",
        "unlocked": false
      },
      {
        "id": 11838,
        "name": "Saradomin sword",
        "unlocked": false
      },
      {
        "id": 11830,
        "name": "Armadyl chainskirt",
        "unlocked": false
      },
      {
        "id": 11836,
        "name": "Bandos boots",
        "unlocked": false
      },
      {
        "id": 11791,
        "name": "Staff of the dead",
        "unlocked": false
      },
      {
        "id": 11816,
        "name": "Zamorak hilt",
        "unlocked": false
      },
      {
        "id": 13256,
        "name": "Saradomin's light",
        "unlocked": false
      },
      {
        "id": 11818,
        "name": "Godsword shard 1",
        "unlocked": false
      },
      {
        "id": 11820,
        "name": "Godsword shard 2",
        "unlocked": false
      },
      {
        "id": 11822,
        "name": "Godsword shard 3",
        "unlocked": false
      },
      {
        "id": 11814,
        "name": "Saradomin hilt",
        "unlocked": false
      }
    ]
  };

  public static readonly ZULRAH: Category = {
    "name": "Zulrah",
    locked: true,
    "items": [
      {
        "id": 12922,
        "name": "Tanzanite fang",
        "unlocked": false
      },
      {
        "id": 12932,
        "name": "Magic fang",
        "unlocked": false
      },
      {
        "id": 12927,
        "name": "Serpentine visage",
        "unlocked": false
      },
      {
        "id": 13200,
        "name": "Tanzanite mutagen",
        "unlocked": false
      },
      {
        "id": 13201,
        "name": "Magma mutagen",
        "unlocked": false
      }
    ]
  };

  public static readonly CERBERUS: Category = {
    "name": "Cerberus",
    locked: true,
    "items": [{
      "id": 13231,
      "name": "Primordial crystal",
      "unlocked": false
    }, {
      "id": 13227,
      "name": "Eternal crystal",
      "unlocked": false
    }, {
      "id": 13229,
      "name": "Pegasian crystal",
      "unlocked": false
    }, {
      "id": 13233,
      "name": "Smouldering stone",
      "unlocked": false
    }],
  };

  public static readonly SKILLCAPES: Category = {
    "name": "Accomplishment Capes",
    locked: true,
    "items": [
      {
        "id": 9748,
        "name": "Attack cape(t)",
        "unlocked": false
      },
      {
        "id": 9751,
        "name": "Strength cape(t)",
        "unlocked": false
      },
      {
        "id": 9754,
        "name": "Defence cape(t)",
        "unlocked": false
      },
      {
        "id": 9757,
        "name": "Ranging cape(t)",
        "unlocked": false
      },
      {
        "id": 9760,
        "name": "Prayer cape(t)",
        "unlocked": false
      },
      {
        "id": 9763,
        "name": "Magic cape(t)",
        "unlocked": false
      },
      {
        "id": 9766,
        "name": "Runecraft cape(t)",
        "unlocked": false
      },
      {
        "id": 9790,
        "name": "Construct. cape(t)",
        "unlocked": false
      },
      {
        "id": 9769,
        "name": "Hitpoints cape(t)",
        "unlocked": false
      },
      {
        "id": 9772,
        "name": "Agility cape(t)",
        "unlocked": false
      },
      {
        "id": 9775,
        "name": "Herblore cape(t)",
        "unlocked": false
      },
      {
        "id": 9778,
        "name": "Thieving cape(t)",
        "unlocked": false
      },
      {
        "id": 9781,
        "name": "Crafting cape(t)",
        "unlocked": false
      },
      {
        "id": 9784,
        "name": "Fletching cape(t)",
        "unlocked": false
      },
      {
        "id": 9787,
        "name": "Slayer cape(t)",
        "unlocked": false
      },
      {
        "id": 9949,
        "name": "Hunter cape(t)",
        "unlocked": false
      },
      {
        "id": 9793,
        "name": "Mining cape(t)",
        "unlocked": false
      },
      {
        "id": 9796,
        "name": "Smithing cape(t)",
        "unlocked": false
      },
      {
        "id": 9799,
        "name": "Fishing cape(t)",
        "unlocked": false
      },
      {
        "id": 9802,
        "name": "Cooking cape(t)",
        "unlocked": false
      },
      {
        "id": 9805,
        "name": "Firemaking cape(t)",
        "unlocked": false
      },
      {
        "id": 9808,
        "name": "Woodcut. cape(t)",
        "unlocked": false
      },
      {
        "id": 9811,
        "name": "Farming cape(t)",
        "unlocked": false
      },
      {
        "id": 9813,
        "name": "Quest point cape",
        "unlocked": false
      },
      {
        "id": 13221,
        "name": "Music cape",
        "unlocked": false
      },
      {
        "id": 19476,
        "name": "Achievement diary cape",
        "unlocked": false
      }
    ]
  };

  public static readonly BARROWS: Category = {
    "name": "Barrows",
    locked: true,
    "items": [
      {
        "id": 4708,
        "name": "Ahrim's hood",
        "unlocked": false
      },
      {
        "id": 4716,
        "name": "Dharok's helm",
        "unlocked": false
      },
      {
        "id": 4724,
        "name": "Guthan's helm",
        "unlocked": false
      },
      {
        "id": 4732,
        "name": "Karil's coif",
        "unlocked": false
      },
      {
        "id": 4745,
        "name": "Torag's helm",
        "unlocked": false
      },
      {
        "id": 4753,
        "name": "Verac's helm",
        "unlocked": false
      },
      {
        "id": 4712,
        "name": "Ahrim's robetop",
        "unlocked": false
      },
      {
        "id": 4720,
        "name": "Dharok's platebody",
        "unlocked": false
      },
      {
        "id": 4728,
        "name": "Guthan's platebody",
        "unlocked": false
      },
      {
        "id": 4736,
        "name": "Karil's leathertop",
        "unlocked": false
      },
      {
        "id": 4749,
        "name": "Torag's platebody",
        "unlocked": false
      },
      {
        "id": 4757,
        "name": "Verac's brassard",
        "unlocked": false
      },
      {
        "id": 4714,
        "name": "Ahrim's robeskirt",
        "unlocked": false
      },
      {
        "id": 4722,
        "name": "Dharok's platelegs",
        "unlocked": false
      },
      {
        "id": 4730,
        "name": "Guthan's chainskirt",
        "unlocked": false
      },
      {
        "id": 4738,
        "name": "Karil's leatherskirt",
        "unlocked": false
      },
      {
        "id": 4751,
        "name": "Torag's platelegs",
        "unlocked": false
      },
      {
        "id": 4759,
        "name": "Verac's plateskirt",
        "unlocked": false
      },
      {
        "id": 4710,
        "name": "Ahrim's staff",
        "unlocked": false
      },
      {
        "id": 4718,
        "name": "Dharok's greataxe",
        "unlocked": false
      },
      {
        "id": 4726,
        "name": "Guthan's warspear",
        "unlocked": false
      },
      {
        "id": 4734,
        "name": "Karil's crossbow",
        "unlocked": false
      },
      {
        "id": 4747,
        "name": "Torag's hammers",
        "unlocked": false
      },
      {
        "id": 4755,
        "name": "Verac's flail",
        "unlocked": false
      }
    ],
  };

  public static readonly DIARIES = {
    "name": "Diaries",
    locked: true,
    "items": [
      {
        "id": 13121,
        "name": "Ardougne cloak 1",
        "unlocked": false
      },
      {
        "id": 13122,
        "name": "Ardougne cloak 2",
        "unlocked": false
      },
      {
        "id": 13123,
        "name": "Ardougne cloak 3",
        "unlocked": false
      },
      {
        "id": 13124,
        "name": "Ardougne cloak 4",
        "unlocked": false
      },
      {
        "id": 13133,
        "name": "Desert amulet 1",
        "unlocked": false
      },
      {
        "id": 13134,
        "name": "Desert amulet 2",
        "unlocked": false
      },
      {
        "id": 13135,
        "name": "Desert amulet 3",
        "unlocked": false
      },
      {
        "id": 13136,
        "name": "Desert amulet 4",
        "unlocked": false
      },
      {
        "id": 13117,
        "name": "Falador shield 1",
        "unlocked": false
      },
      {
        "id": 13118,
        "name": "Falador shield 2",
        "unlocked": false
      },
      {
        "id": 13119,
        "name": "Falador shield 3",
        "unlocked": false
      },
      {
        "id": 13120,
        "name": "Falador shield 4",
        "unlocked": false
      },
      {
        "id": 13129,
        "name": "Fremennik sea boots 1",
        "unlocked": false
      },
      {
        "id": 13130,
        "name": "Fremennik sea boots 2",
        "unlocked": false
      },
      {
        "id": 13131,
        "name": "Fremennik sea boots 3",
        "unlocked": false
      },
      {
        "id": 13132,
        "name": "Fremennik sea boots 4",
        "unlocked": false
      },
      {
        "id": 13137,
        "name": "Kandarin headgear 1",
        "unlocked": false
      },
      {
        "id": 13138,
        "name": "Kandarin headgear 2",
        "unlocked": false
      },
      {
        "id": 13139,
        "name": "Kandarin headgear 3",
        "unlocked": false
      },
      {
        "id": 13140,
        "name": "Kandarin headgear 4",
        "unlocked": false
      },
      {
        "id": 11136,
        "name": "Karamja gloves 1",
        "unlocked": false
      },
      {
        "id": 11138,
        "name": "Karamja gloves 2",
        "unlocked": false
      },
      {
        "id": 11140,
        "name": "Karamja gloves 3",
        "unlocked": false
      },
      {
        "id": 13103,
        "name": "Karamja gloves 4",
        "unlocked": false
      },
      {
        "id": 22941,
        "name": "Rada's blessing 1",
        "unlocked": false
      },
      {
        "id": 22943,
        "name": "Rada's blessing 2",
        "unlocked": false
      },
      {
        "id": 22945,
        "name": "Rada's blessing 3",
        "unlocked": false
      },
      {
        "id": 22947,
        "name": "Rada's blessing 4",
        "unlocked": false
      },
      {
        "id": 13125,
        "name": "Explorer's ring 1",
        "unlocked": false
      },
      {
        "id": 13126,
        "name": "Explorer's ring 2",
        "unlocked": false
      },
      {
        "id": 13127,
        "name": "Explorer's ring 3",
        "unlocked": false
      },
      {
        "id": 13128,
        "name": "Explorer's ring 4",
        "unlocked": false
      },
      {
        "id": 13112,
        "name": "Morytania legs 1",
        "unlocked": false
      },
      {
        "id": 13113,
        "name": "Morytania legs 2",
        "unlocked": false
      },
      {
        "id": 13114,
        "name": "Morytania legs 3",
        "unlocked": false
      },
      {
        "id": 13115,
        "name": "Morytania legs 4",
        "unlocked": false
      },
      {
        "id": 13104,
        "name": "Varrock armour 1",
        "unlocked": false
      },
      {
        "id": 13105,
        "name": "Varrock armour 2",
        "unlocked": false
      },
      {
        "id": 13106,
        "name": "Varrock armour 3",
        "unlocked": false
      },
      {
        "id": 13107,
        "name": "Varrock armour 4",
        "unlocked": false
      },
      {
        "id": 13141,
        "name": "Western banner 1",
        "unlocked": false
      },
      {
        "id": 13142,
        "name": "Western banner 2",
        "unlocked": false
      },
      {
        "id": 13143,
        "name": "Western banner 3",
        "unlocked": false
      },
      {
        "id": 13144,
        "name": "Western banner 4",
        "unlocked": false
      },
      {
        "id": 13108,
        "name": "Wilderness sword 1",
        "unlocked": false
      },
      {
        "id": 13109,
        "name": "Wilderness sword 2",
        "unlocked": false
      },
      {
        "id": 13110,
        "name": "Wilderness sword 3",
        "unlocked": false
      },
      {
        "id": 13111,
        "name": "Wilderness sword 4",
        "unlocked": false
      }
    ],
  };

  public static readonly DKS: Category = {
    "name": "DKs",
    locked: true,
    "items": [{
      "id": 6737,
      "name": "Berserker ring",
      "unlocked": false
    }, {
      "id": 6735,
      "name": "Warrior ring",
      "unlocked": false
    }, {
      "id": 6733,
      "name": "Archers ring",
      "unlocked": false
    }, {
      "id": 6731,
      "name": "Seers ring",
      "unlocked": false
    }, {
      "id": 6562,
      "name": "Mud battlestaff",
      "unlocked": false
    }, {
      "id": 6724,
      "name": "Seercull",
      "unlocked": false
    }]
  };

  public static readonly ZENYTES: Category = {
    "name": "Zenytes",
    locked: true,
    "items": [{
      "id": 19553,
      "name": "Amulet of torture",
      "unlocked": false
    }, {
      "id": 19547,
      "name": "Necklace of anguish",
      "unlocked": false
    }, {
      "id": 19544,
      "name": "Tormented bracelet",
      "unlocked": false
    }, {
      "id": 19550,
      "name": "Ring of suffering",
      "unlocked": false
    }],
  };

  public static readonly HYDRA: Category = {
    "name": "Hydra",
    locked: true,
    "items": [{
      "id": 22966,
      "name": "Hydra's claw",
      "unlocked": false
    }, {
      "id": 22983,
      "name": "Hydra leather",
      "unlocked": false
    }, {
      "id": 22969,
      "name": "Hydra's heart",
      "unlocked": false
    }, {
      "id": 22971,
      "name": "Hydra's fang",
      "unlocked": false
    }, {
      "id": 22973,
      "name": "Hydra's eye",
      "unlocked": false
    }, {
      "id": 22988,
      "name": "Hydra tail",
      "unlocked": false
    }, {
      "id": 23077,
      "name": "Alchemical hydra heads",
      "unlocked": false
    }],
  };

  public static readonly REVS: Category = {
    "name": "Revs",
    locked: true,
    "items": [{
      "id": 22542,
      "name": "Viggora's chainmace (u)",
      "unlocked": false
    }, {
      "id": 22547,
      "name": "Craw's bow (u)",
      "unlocked": false
    }, {
      "id": 22552,
      "name": "Thammaron's sceptre (u)",
      "unlocked": false
    }],
  };

  public static readonly JARS: Category = {
    "name": "Jars",
    locked: true,
    "items": [{
      "id": 12007,
      "name": "Jar of dirt",
      "unlocked": false
    }, {
      "id": 12885,
      "name": "Jar of sand",
      "unlocked": false
    }, {
      "id": 12936,
      "name": "Jar of swamp",
      "unlocked": false
    }, {
      "id": 13245,
      "name": "Jar of souls",
      "unlocked": false
    }, {
      "id": 13277,
      "name": "Jar of miasma",
      "unlocked": false
    }, {
      "id": 19701,
      "name": "Jar of darkness",
      "unlocked": false
    }, {
      "id": 21745,
      "name": "Jar of stone",
      "unlocked": false
    }, {
      "id": 23525,
      "name": "Jar of eyes",
      "unlocked": false
    }, {
      "id": 24495,
      "name": "Jar of dreams",
      "unlocked": false
    }, {
      "id": 23064,
      "name": "Jar of chemicals",
      "unlocked": false
    }],
  };

  public static readonly CHAMP_SCROLLS: Category = {
    "name": "Champ Scrolls",
    locked: true,
    "items": [{
      "id": 6798,
      "name": "Earth warrior champion scroll",
      "unlocked": false
    }, {
      "id": 6799,
      "name": "Ghoul champion scroll",
      "unlocked": false
    }, {
      "id": 6800,
      "name": "Giant champion scroll",
      "unlocked": false
    }, {
      "id": 6801,
      "name": "Goblin champion scroll",
      "unlocked": false
    }, {
      "id": 6802,
      "name": "Hobgoblin champion scroll",
      "unlocked": false
    }, {
      "id": 6803,
      "name": "Imp champion scroll",
      "unlocked": false
    }, {
      "id": 6804,
      "name": "Jogre champion scroll",
      "unlocked": false
    }, {
      "id": 6805,
      "name": "Lesser demon champion scroll",
      "unlocked": false
    }, {
      "id": 6806,
      "name": "Skeleton champion scroll",
      "unlocked": false
    }, {
      "id": 6807,
      "name": "Zombie champion scroll",
      "unlocked": false
    }]
  };

  public static readonly SLAYER: Category = {
    "name": "Slayer",
    locked: true,
    "items": [{
      "id": 20724,
      "name": "Imbued heart",
      "unlocked": false
    }, {
      "id": 21270,
      "name": "Eternal gem",
      "unlocked": false
    }, {
      "id": 20736,
      "name": "Dust battlestaff",
      "unlocked": false
    }, {
      "id": 20730,
      "name": "Mist battlestaff",
      "unlocked": false
    }, {
      "id": 12002,
      "name": "Occult necklace",
      "unlocked": false
    }, {
      "id": 11908,
      "name": "Uncharged trident",
      "unlocked": false
    }, {
      "id": 12004,
      "name": "Kraken tentacle",
      "unlocked": false
    }, {
      "id": 11840,
      "name": "Dragon boots",
      "unlocked": false
    }, {
      "id": 8901,
      "name": "Black mask (10)",
      "unlocked": false
    }, {
      "id": 24268,
      "name": "Basilisk jaw",
      "unlocked": false
    }],
  };

  public static readonly SKILLING: Category = {
    "name": "Skilling",
    "items": [{"id": 11850, "name": "Graceful hood", "unlocked": false}, {
      "id": 11854,
      "name": "Graceful top",
      "unlocked": false
    }, {"id": 11856, "name": "Graceful legs", "unlocked": false}, {
      "id": 11860,
      "name": "Graceful boots",
      "unlocked": false
    }, {"id": 11858, "name": "Graceful gloves", "unlocked": false}, {
      "id": 11852,
      "name": "Graceful cape",
      "unlocked": false
    }, {"id": 5554, "name": "Rogue mask", "unlocked": false}, {
      "id": 5553,
      "name": "Rogue top",
      "unlocked": false
    }, {"id": 5555, "name": "Rogue trousers", "unlocked": false}, {
      "id": 5557,
      "name": "Rogue boots",
      "unlocked": false
    }, {"id": 5556, "name": "Rogue gloves", "unlocked": false}, {
      "id": 10941,
      "name": "Lumberjack hat",
      "unlocked": false
    }, {"id": 10939, "name": "Lumberjack top", "unlocked": false}, {
      "id": 10940,
      "name": "Lumberjack legs",
      "unlocked": false
    }, {"id": 10933, "name": "Lumberjack boots", "unlocked": false}, {
      "id": 13258,
      "name": "Angler hat",
      "unlocked": false
    }, {"id": 13259, "name": "Angler top", "unlocked": false}, {
      "id": 13260,
      "name": "Angler waders",
      "unlocked": false
    }, {"id": 13261, "name": "Angler boots", "unlocked": false}, {
      "id": 20708,
      "name": "Pyromancer hood",
      "unlocked": false
    }, {"id": 20704, "name": "Pyromancer garb", "unlocked": false}, {
      "id": 20706,
      "name": "Pyromancer robe",
      "unlocked": false
    }, {"id": 20710, "name": "Pyromancer boots", "unlocked": false}, {
      "id": 12013,
      "name": "Prospector helmet",
      "unlocked": false
    }, {"id": 12014, "name": "Prospector jacket", "unlocked": false}, {
      "id": 12015,
      "name": "Prospector legs",
      "unlocked": false
    }, {"id": 12016, "name": "Prospector boots", "unlocked": false}, {
      "id": 13646,
      "name": "Farmer's strawhat",
      "unlocked": false
    }, {"id": 13642, "name": "Farmer's jacket", "unlocked": false}, {
      "id": 13640,
      "name": "Farmer's boro trousers",
      "unlocked": false
    }, {"id": 13644, "name": "Farmer's boots", "unlocked": false}],
    "locked": false
  };

  public static readonly SLAYER_HELMS: Category = {
    "name": "Slayer Helms",
    locked: true,
    "items": [{
      "id": 14003,
      "name": "Slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 19642,
      "name": "Black slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 19646,
      "name": "Green slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 19650,
      "name": "Red slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 21267,
      "name": "Purple slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 21891,
      "name": "Turquoise slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 23076,
      "name": "Hydra slayer helmet (i)",
      "unlocked": false
    }, {
      "id": 24445,
      "name": "Twisted slayer helmet (i)",
      "unlocked": false
    }],
  };

  public static readonly DEFAULT_CATEGORIES: Category[] = [
    DefaultCategories.TOB,
    DefaultCategories.COX,
    DefaultCategories.GWD,
    DefaultCategories.HYDRA,
    DefaultCategories.BARROWS,
    DefaultCategories.CERBERUS,
    DefaultCategories.ZENYTES,
    DefaultCategories.SLAYER_HELMS,
    DefaultCategories.SKILLCAPES,
    DefaultCategories.SLAYER,
    DefaultCategories.PETS,
    DefaultCategories.ZULRAH,
    DefaultCategories.JARS,
    DefaultCategories.CHAMP_SCROLLS,
    DefaultCategories.DKS,
    DefaultCategories.SKILLING,
    DefaultCategories.DIARIES,
    DefaultCategories.REVS,
  ];
}

